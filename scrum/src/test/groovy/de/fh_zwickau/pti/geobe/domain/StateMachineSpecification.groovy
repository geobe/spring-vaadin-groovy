...
class StateMachineSpecification extends Specification {
    ...
    def 'the basic statemachine behaviour'() {
        ...
        sm.addAction(State.A, State.B, Event.X, { log.info('A-X->B') })
        sm.addAction(State.A, State.C, Event.Y, { log.info('A-Y->C') })
        sm.addAction(State.A, State.A, Event.Z, { log.info('A-Z->A') })
        sm.addAction(State.B, State.A, Event.Z, { log.info('B-Z->A') })
        sm.addAction(State.C, State.A, Event.Z, { log.info('C-Z->A') })
        sm.addAction(State.D, State.A, Event.Z, { log.info('D-Z->A') })
        sm.addAction(State.B, State.D, Event.X, { log.info('B-X->D') })
        sm.addAction(State.C, State.D, Event.X, { log.info('C-X->!D but B'); State.B })
    ...
    def 'state machine executes closure with parameters'() {
        ...
        DialogStateMachine<State, Event> sm = new DialogStateMachine<>(State.A)
        def r1, r2
        sm.addAction(State.A, State.B, Event.X, { p1 -> r1 = p1[0]; r2 = p1[1] })
